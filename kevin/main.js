...
ethan_getEFDdata = async function(cohort_list_arg, gene_list_arg, plot_type) {
    ...
    if (plot_type == 'histogram') {
      var plotToReturn = getDataToPlotForHistogram(expressionArray, cohort_list_arg, gene_list_arg);
    } else if (plot_type == 'heatmap') {
      var plotToReturn = getDataToPlotForHeatmap(expressionArray, cohort_list_arg, gene_list_arg);
    }
    return await plotToReturn;  
  };
  ...
  addElement = function(newDivID, oldDivID) { 
    ...
    // newDiv.setAttribute("style", "height: 15%");
    // newDiv.setAttribute("style", "width: 45%"); 
